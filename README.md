...
14. useFormStatus
Позволяет отображать индикатор загрузки во время выполнения действия (Server Action)
https://nextjs.org/docs/app/guides/forms


15. useActionState
Позволяет обрабатывать серверные действия
const [state, action, pending] = useActionState(createPost, false)
значение состояния любого типа
функция, которая запускает действие и обновляет состояние.
находится ли действие в процессе выполнения.


16. Валидация должна происходить и на клиенте и на сервере
на клиенте - пользователь быстрее получал отклие
на сервере - для безопасности


17. ZOD - библиотека для валидации данных
npm install zod
lib/validator.ts

18. server actions
Позволяет не только работать с формами, но и проводить любые вычисления/логику на сервере,


19. useTranslation
Позволяет обновлять состояние без блокирования UI
Позволяет получить состояние без ожидания pending
```
